...
namespace Squalr.Theme.Controls
{
	...
	public abstract class LayoutFloatingWindowControl : Window, ILayoutControl
	{
		...
		internal virtual void UpdateThemeResources(ThemeBase oldTheme = null)
		{
			...
			if (manager?.ThemeBase == null) return;
			if (manager.ThemeBase is DictionaryTheme dictionaryTheme)
			{
				currentThemeResourceDictionary = dictionaryTheme.ThemeResourceDictionary;
				Resources.MergedDictionaries.Add(currentThemeResourceDictionary);
			}
			else
				Resources.MergedDictionaries.Add(new ResourceDictionary { Source = manager.ThemeBase.GetResourceUri() });
		}
